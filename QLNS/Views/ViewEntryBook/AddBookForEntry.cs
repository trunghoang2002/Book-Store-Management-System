...
{
    public partial class AddBookForEntry : Form
    {
        public delegate void Mydel();
        public Mydel d;
        public AddBookForEntry()
        {
            InitializeComponent();
        ...
        private void LoadData(string textSearch = "", int idCategory = 0)
        {
            ...
            if (BLL_Book.Instance.GetListBookViewAdmin(textSearch, idCategory).Count > 0)
            {
        ...
        private void cBtnAddBook_Click(object sender, EventArgs e)
        {
            if (dataGridViewBookOrder.SelectedRows.Count == 1)
            {
                ...
                else
                {
                    ...
                    if (BLL_Book.Instance.CheckBookInList(bookID))
                    {
                        DialogResult result = MessageBox.Show("This book is already in list. Do you want to continue?", "Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                        if (result == DialogResult.Cancel)
                            return;
                    }
                    BLL_Book.Instance.AddBookToList(bookID, quantity);
                    d();
                    this.Close();
                }
            }
